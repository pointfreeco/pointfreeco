...
let view: View<Prelude.Unit> = simplePageLayout(aboutView)
  .contramap { _ in SimplePageLayoutData(currentUser: nil, data: unit, title: "") }


AppEnvironment.push(const(.mock))

let htmlStr = render(view.view(unit))

let webView = WKWebView(frame: .init(x: 0, y: 0, width: 580, height: 750))
webView.loadHTMLString(htmlStr, baseURL: nil)
