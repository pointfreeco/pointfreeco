...
public func sendWelcomeEmails() async throws {
  ...
  await notifyError("Welcome emails failed") {
    async let emails1 = database.fetchUsersToWelcome(registeredWeeksAgo: 1).map(welcomeEmail1_V2)
...
func welcomeEmail1_V2(_ user: User) -> Email {
      ...
      TableRow {
        TableData {
          EmailMarkdown {
            ...
            hope you've learned a thing or two new about Swift, and maybe even introduced a learning 
            into your codebase! We'd love to [have you as a 
            ...
            if user.episodeCreditCount > 0 {
              ...
              see *any* subscriber-only episode, completely for free! Just visit [our
              site](\(siteRouter.url(for: .home))), go to an episode, and click the 
              "\(useCreditCTA)" button!
...
private struct HostSignOff: HTML {
  var body: some HTML {
    HTMLMarkdown {
      """
      Your hosts,
...
func welcomeEmail2Content(user: User) -> Node {
  ...
  let freeEpisodeLinks = episodes()
    ...
    .map {
      """
      * [\($0.fullTitle)](\(siteRouter.url(for: .episodes(.show($0)))))
      """
    }
    .joined(separator: "\n")
