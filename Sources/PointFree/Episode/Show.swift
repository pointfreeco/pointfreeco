...
let episodeResponse: M<Tuple5<Either<String, Episode.Id>, User?, SubscriberState, AppRoute?, Episode.Collection.Slug?>> =
...
func useCreditResponse<Z>(
  conn: Conn<StatusLineOpen, T5<Either<String, Episode.Id>, User?, SubscriberState, AppRoute?, Z>>
...
private func fetchEpisodeForParam<Z>(
  middleware: @escaping M<T5<Episode, User?, SubscriberState, AppRoute?, Z>>
) -> M<T5<Either<String, Episode.Id>, User?, SubscriberState, AppRoute?, Z>> {
  middleware
    |> filterMap(
