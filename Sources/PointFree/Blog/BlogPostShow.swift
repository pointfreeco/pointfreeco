...
let blogPostShowMiddleware
  = fetchBlogPostForParam
    ...
    >>> respond(
      ...
      layoutData: { (post: BlogPost, currentUser: User?, subscriberState: SubscriberState, currentRoute: AppRoute?) in
...
private let fetchBlogPostForParam
  : MT<
  Tuple4<Either<String, BlogPost.Id>, User?, SubscriberState, AppRoute?>,
