...
let removeTeammateMiddleware =
  ...
  >=> redirect(
    to: .account(.index), headersMiddleware: flash(.notice, "That teammate has been removed."))
...
private let requireTeammate: MT<Tuple2<User.Id, User>, Tuple2<User, User>> = filterMap(
  over1 {
    ...
    >>> map(require1),
  or: redirect(
    to: .account(.index), headersMiddleware: flash(.error, "Could not find that teammate."))
)
