...
private let creditsView = View<AccountData> { data in
...
private let episodeCreditsView = View<[Database.EpisodeCredit]> { credits in
...
private let profileRowView = View<AccountData> { data -> Node in
  ...
  let extraInvoiceInfoFields: Node = !data.isSubscriptionOwner ? [] : [
...
private let emailSettingCheckboxes = View<([Database.EmailSetting], SubscriberState)> { currentEmailSettings, subscriberState in
  ...
  return [
    ...
    p(["Receive email for:"]),
    p(
      [`class`([Class.padding([.mobile: [.left: 1]])])],
      .fragment(
...
private let subscriptionOverview = View<AccountData> { data in
...
private let privateRssFeed = View<AccountData> { data in
  ...
  return gridRow(
    gridColumn(
      ...
      [HtmlCssSupport.style(margin(leftRight: .auto))],
      div(
        [
          `class`([
...
"""),
        p(
          ...
          a(
            [`class`([Class.pf.type.underlineLink]), href("mailto:support@pointfree.co")],
            "email us"
          ),
...
private let subscriptionOwnerOverview = View<AccountData> { data in
...
private let subscriptionTeammateOverview = View<AccountData> { data in
  guard let subscription = data.stripeSubscription else { return [] }
  ...
  return gridRow([`class`([Class.padding([.mobile: [.bottom: 4]])])],
    gridColumn(
      ...
      div(
        ...
        form(
          [action(path(to: .team(.leave))), method(.post)],
          input(
            [
...
private let subscriptionPlanRows = View<(Stripe.Subscription, Stripe.Invoice?)> { subscription, upcomingInvoice -> Node in
  ...
  let planRow = gridRow(
    gridColumn(sizes: [.mobile: 3], p(div("Plan"))),
    gridColumn(
      sizes: [.mobile: 9],
      gridRow(
        gridColumn(
          ...
          div(
            [`class`([Class.padding([.mobile: [.leftRight: 1]])])],
  ...
  let statusRow = gridRow(
    gridColumn(sizes: [.mobile: 3], p(div("Status"))),
    gridColumn(
      sizes: [.mobile: 9],
      gridRow(
        gridColumn(
  ...
  let nextBillingRow = subscription.cancelAtPeriodEnd || subscription.status == .canceled
    ? nil
    : gridRow(
      gridColumn(sizes: [.mobile: 3], p(div("Next billing"))),
      gridColumn(
        sizes: [.mobile: 9],
        div(
          [`class`([Class.padding([.mobile: [.leftRight: 1]])])],
  ...
  return div(
    ...
    [planRow, statusRow, nextBillingRow ?? [], discountRow ?? []]
  )
}
...
private let subscriptionTeamRow = View<AccountData> { data in
  ...
  return gridRow(
    [`class`([subscriptionInfoRowClass])],
...
private let teammateRowView = View<(Database.User, Database.User)> { currentUser, teammate -> Node in
  ...
  return gridRow(
    ...
    gridColumn(
      sizes: [.mobile: 4], [`class`([Class.grid.end(.desktop)])],
      form(
  ...
  return gridRow(
    ...
    gridColumn(
      ...
      div(
        ...
        .fragment(invites.map(inviteRowView.view))
      )
    )
  )
}
...
private let inviteRowView = View<Database.TeamInvite> { invite in
  gridRow(
    ...
    gridColumn(
      sizes: [.mobile: 12, .desktop: 6], [`class`([Class.grid.end(.desktop)])],
      form(
...
private let subscriptionInviteMoreRowView = View<(Stripe.Subscription?, [Database.TeamInvite], [Database.User])> { subscription, invites, teammates in
  ...
  return gridRow(
    [`class`([subscriptionInfoRowClass])],
    gridColumn(sizes: [.mobile: 3], div(p("Invite more"))),
    gridColumn(
      sizes: [.mobile: 9],
      div(
        [`class`([Class.padding([.mobile: [.leftRight: 1]])])],
        ...
        form(
          [action(path(to: .invite(.send(nil)))), method(.post), `class`([Class.flex.flex])],
          input([
...
private let subscriptionPaymentInfoView = View<Stripe.Subscription> { subscription in
  guard let card = subscription.customer.right?.sources.data.first else { return [] }

  return gridRow(
    [`class`([subscriptionInfoRowClass])],
    gridColumn(sizes: [.mobile: 3], div(p("Payment"))),
    gridColumn(
      sizes: [.desktop: 9],
      gridRow(
        gridColumn(
...
private let logoutView = View<Prelude.Unit> { _ in
  gridRow(
    gridColumn(
      sizes: [.mobile: 12],
      a(
        [`class`([Class.pf.components.button(color: .black)]), href(path(to: .logout))],
        "Logout"
      )
    )
  )
