...
private let validateEmail: MT<Tuple2<User, ProfileData>, Tuple2<User, ProfileData>> = filter(
  ...
  or: redirect(
    to: .account(.index), headersMiddleware: flash(.error, "Please enter a valid email."))
)
...
let emailChangeIso: PartialIso<String, (User.Id, EmailAddress)> = payload(
  .uuid >>> .tagged, .tagged)
