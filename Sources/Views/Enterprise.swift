...
public func enterpriseView(_ currentUser: User?, _ account: EnterpriseAccount) -> Node {
  let loggedOutView: Node = [
    ...
    .gitHubLink(
      ...
      type: .white,
      href: pointFreeRouter.path(
        to: .login(redirect: pointFreeRouter.url(to: .enterprise(.landing(account.domain)))))
    ),
  ]
    ...
    .gridRow(
      ...
      .gridColumn(
        ...
        .form(
          attributes: [
            .action(
              pointFreeRouter.path(
                to: .enterprise(.requestInvite(account.domain, .init(email: ""))))),
            .method(.post),
          ],
          .gridRow(
            attributes: [
