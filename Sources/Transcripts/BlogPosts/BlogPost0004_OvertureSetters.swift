import Foundation

extension BlogPost {
  public static let post0004_overtureSetters = Self(
    author: .brandon,
    blurb: """
      Announcing Overture 0.2.0! This release is all about setters: functions that allow us to \
      build complex transformations out of smaller units.
      """,
    coverImage:
      "https://d1iqsrac68iyd8.cloudfront.net/posts/0004-overture-functional-setters/0004-poster-1.jpg",
    id: 4,
    publishedAt: .init(timeIntervalSince1970: 1_526_291_823 + 60 * 60 * 24),
    title: "Overture: Now with Functional Setters"
  )
}
