...
"""#,
        ...
        related: [
          .init(
            blurb: """
This section modeled side-effects using a custom `Effect` type built from scratch. It turns out that any type that models asynchrony (such as reactive and promise libaries) can be used to model effects. So, we refactor the composable architecture to take advantage of Combine, Apple's reactive framework.
""",
            content: .episodes([
...
"""#,
          ...
          .init(
            blurb: #"""
Although the environment technique is powerful, and can help you get test coverage in an application almost immediately, we can make it even better in the composable architecture. By baking the notion of an environment directly into the core types of the architecture we will make our dependencies more controllable and more understandable, all without sacrificing composability.
"""#,
            content: .episodes([
