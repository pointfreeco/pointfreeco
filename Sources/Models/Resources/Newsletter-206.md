...
**ComposableArchitecture 2.0** is our biggest release ever, and is a fundamental redesign of how
features are built, how side effects are managed, how features communicate, how features are
tested, and a lot more. The result is dramatically less boilerplate, a more intuitive mental model,
and testing that is more powerful than ever.
...
`TestStore` uses [DebugSnapshots](/blog/posts/205-beta-preview-debugsnapshots) under the hood, and
`@DebugSnapshot` automatically skips private properties. You can keep internal bookkeeping in
private state without cluttering your tests.
...
* We integrated the `TestStore` with our new
[DebugSnapshots](/blog/posts/205-beta-preview-debugsnapshots) library to allow for testing without
making the `State` of your features `Equatable`, and you can even store reference types in `State`
without hurting testability.
