...
```swift
Trip.where { $0.location.jsonExtract(\.longitude).lt(0) }
// SELECT … FROM "trips"
...
// UPDATE "profiles"
// SET "author" = json_set("profiles"."author", '$."name"', 'Blob')

Profile.update { $0.tags = $0.tags.jsonAppend("new") }
// UPDATE "profiles"
// SET "tags" = json_insert("profiles"."tags", '$[#]', 'new')
```
...
//   WHERE json_extract("json_each"."value", '$."latitude"') > 0
// )
```
...
// WHERE EXISTS (
//   SELECT … FROM json_each("reminders"."tags")
